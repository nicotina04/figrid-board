...
struct Stats {
    ...
    candidate_tests: u64,
    mismatches: u64,
    stored_mismatches: usize,
}

fn main() -> Result<(), String> {
    let args = parse_args()?;
    let mut stats = Stats::default();
    let mut mismatches = File::create(&args.out_mismatches_jsonl).map_err(|e| {
        format!(
            "failed to create {}: {e}",
            args.out_mismatches_jsonl.display()
        )
    })?;

    for path in &args.positions_jsonl {
        sweep_position_jsonl(path, &args, &mut stats, &mut mismatches)?;
    }
    if let Some(path) = &args.games_jsonl {
        sweep_games_jsonl(path, &args, &mut stats, &mut mismatches)?;
    }

    let report = json!({
        "format": "rq572-classify-equivalence-v1",
        ...
        "stats": {
            ...
            "stored_mismatches": stats.stored_mismatches,
        },
    ...
    println!(
        "rq572-classify-equivalence: position_boards={} game_boards={} candidate_tests={} mismatches={}",
        stats.position_boards, stats.game_boards, stats.candidate_tests, stats.mismatches
    );
    if stats.mismatches > 0 {
        return Err(format!(
            "slow/fast classify mismatch count: {} (see {})",
            stats.mismatches,
            args.out_mismatches_jsonl.display()
        ));
    }
...
fn sweep_position_jsonl(
    ...
    for (line_no, line) in BufReader::new(file).lines().enumerate() {
        ...
        sweep_board(&board, &label, args, stats, mismatches)?;
    ...
    for mv in 0..NUM_CELLS {
        ...
        for side in [Stone::Black, Stone::White] {
            for flags in FLAG_COMBOS {
                ...
                if slow != fast {
                    stats.mismatches += 1;
                    if stats.stored_mismatches < args.max_mismatches {
                        stats.stored_mismatches += 1;
                        let rec = mismatch_record(board, label, mv, side, flags, slow, fast);
                        writeln!(mismatches, "{}", serde_json::to_string(&rec).unwrap())
                            .map_err(|e| format!("failed to write mismatch jsonl: {e}"))?;
                    }
                }
            }
        }
...
fn parse_args() -> Result<Args, String> {
    ...
    let mut out_mismatches_jsonl = None;
    let mut max_transitions = 100_000usize;
    let mut max_mismatches = 128usize;
    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            ...
            "--out-mismatches-jsonl" => {
                out_mismatches_jsonl = Some(PathBuf::from(next_arg(&mut it, &arg)?))
            }
            "--max-transitions" => {
            ...
            "--max-mismatches" => {
                max_mismatches = next_arg(&mut it, &arg)?
                    .parse()
    ...
    if positions_jsonl.is_empty() && games_jsonl.is_none() {
        return Err("missing --positions-jsonl or --games-jsonl".to_string());
    }
    Ok(Args {
        positions_jsonl,
        games_jsonl,
        out_json: out_json.ok_or("missing --out-json")?,
        out_mismatches_jsonl: out_mismatches_jsonl.ok_or("missing --out-mismatches-jsonl")?,
        max_transitions,
        max_mismatches,
    })
}
...
fn print_help() {
    eprintln!(
        "usage: rq572-classify-equivalence --positions-jsonl POS.jsonl [--positions-jsonl POS.jsonl] \\\n         [--games-jsonl games.jsonl] --out-json OUT.json --out-mismatches-jsonl OUT.jsonl \\\n         [--max-transitions 100000] [--max-mismatches 128]"
    );
}
