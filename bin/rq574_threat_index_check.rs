use figrid_board::vct::vct_threat_index_transition_check_for_audit;
...
fn main() -> Result<(), String> {
    ...
    'passes: while stats.transitions < args.max_transitions {
        for game in &games {
            ...
            match vct_threat_index_transition_check_for_audit(&game.moves[..take]) {
    ...
    let report = json!({
        "format": "rq574-threat-index-check-v1",
    ...
    println!(
        "rq574-threat-index-check: transitions={} undo_checks={} passed={}",
        stats.transitions, stats.undo_checks, passed
    );
    if passed {
        Ok(())
    } else {
        Err(format!("threat index mismatch: {first_error}"))
    }
}
...
fn print_help() {
    eprintln!(
        "usage: rq574-threat-index-check --games-jsonl games.jsonl --out-json out.json [--max-transitions 100000]"
    );
}
